...
#-------------------------查看資料集長甚麼樣子-------------------------
count = 0
subplot_ = [[0, 0], [0, 1], [1, 0], [1, 1]]
_, ax = plt.subplots(2, 2, figsize=(10, 15))
for img_, _ in train_horses.take(4):
    print(img_)
    ax[subplot_[count][0], subplot_[count][1]].imshow(img_)
    count += 1
plt.show()
#------------------------------------------------------------------
buffer_size = 256
...
kernel_init = keras.initializers.RandomNormal(mean=0.0, stddev=0.02) #隨機生成常態分佈的kernel，均值為0.0，標準差為0.02
...
def normalize_img(img):
    ...
    # 原始img的值介於0~255，除127.5後值會落在0~2之間，減去1之後便能使值控制在-1~1之間
    # 正規化後的圖像在做矩陣運算時會減少model的運算負擔
...
train_horses = (
    #map可以將每一個子元素都套用function，在藉由num_parrallel_calls，去平行運算，如此便能將每個image都做正規劃、resize...等等
...
def downsample(x, filters, activation, kernel_initializer=kernel_init, kernel_size=(3, 3),
    ...
    x = tfa.layers.InstanceNormalization(gamma_initializer = gamma_initializer)(x)
    # 通常在神經網路中最常使用的正則化方法為BN(Batch Normalization)，但BN用在資料集較少的model上時，效果並不理想
    # IN(Instance Normalization)則是為了解決此種問題而提出的方法
    if activation:
        x = activation(x)
    return x
...
def get_discriminator(filters=64, kernel_initializer=kernel_init, num_downsampling=3, name=None):
    img_input = layers.Input(shape=input_img_size, name=name + "_img_input")
    x = layers.Conv2D( filters, (4, 4), strides=(2, 2),
        padding="same", kernel_initializer=kernel_initializer)(img_input)
    ...
    for num_downsample_block in range(3):
        ...
        if num_downsample_block < 2:
            x = downsample( x, filters=num_filters,
                activation=layers.LeakyReLU(0.2), kernel_size=(4, 4), strides=(2, 2))
        else:
            x = downsample( x, filters=num_filters, activation=layers.LeakyReLU(0.2),
...
print("gen_G:",gen_G.summary())
gen_F = get_resnet_generator(name="generator_F")

# Get the discriminators
disc_X = get_discriminator(name="discriminator_X")
print("disc_X:",disc_X.summary())
...
class CycleGan(keras.Model):
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            # Horse to fake zebra 馬生成假的斑馬
            fake_y = self.gen_G(real_x, training=True)
            # Zebra to fake horse -> y2x 斑馬生成假的馬
            fake_x = self.gen_F(real_y, training=True)
            # 再來，藉由生成的假資料圖象，cycle生成原始圖象
...
# Callbacks
plotter = GANMonitor()
checkpoint_filepath = "E:/Neural_Network/CycleGAN/model_checkpoints/cyclegan_checkpoints.{epoch:03d}"
model_checkpoint_callback = keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath)

print("model_summary:",cycle_gan_model)
print("model_checkpoint_callback:", model_checkpoint_callback)
